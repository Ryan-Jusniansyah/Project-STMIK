<x-sidebar>
    ...
    <div class="container mt-3 ms-3 me-3">
        <h2 class="fw-bold mb-3" style="color: #740001;">Edit User</h2>
        <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="name" class="form-label">Name</label>
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Name" value="{{ $user->name }}">
            </div>
            <div class="mb-3">
                ...
                <label for="role" class="select-label">Role</label>
                    <select class="form-select mt-2 @error('role') is-invalid @enderror" aria-label="role" name="roles[]" id="role">
                        ...
                        @foreach ($roles as $value => $label)
                            <option {{ isset($userRole[$value]) ? 'selected' : ''}} value="{{ $value }}">{{ $label }}</option>
                        @endforeach
                    </select>
            </div>
